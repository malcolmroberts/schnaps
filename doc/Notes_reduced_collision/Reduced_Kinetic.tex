...
\ds\dt \f+\vv\cdot\nabla_{\x}\f+\vE\cdot\nabla_{\vv}\f=Q(f,f)\\
\\
\ds \vE=-\nabla\phi\mbox{, }-\triangle \phi=\rho-\rho_0
\end{array}\right.
\end{equation}
with $Q(f,f)$ a collisional operator, $\phi$ the electrical potential. This equation satisfy a conservation for the total energy in the collisionless case.
\begin{lemma}
The equations (\ref{VP}) without collisions satisfy
$$
\frac{d}{dt}\left(\frac12\intx \f\mid \vv\mid^2 d\x d\vv+\frac{1}2\intx \mid\nabla \phi\mid^2 d\x\right)=0
$$
\end{lemma} 
\begin{proof}
...
\begin{align*}
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)+\intx\nabla_{\x}\phi \cdot\vJ=0\\
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)-\intx\phi \nabla_{\x}\cdot\vJ=0\\
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)+\intx\phi \dt \rho=0\\
\end{align*}
Using the Poisson equation we obtain 
\begin{align*}
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)-\intx\phi \dt \triangle \phi=0\\
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)+\dt \intx \nabla_{\x} \phi \cdot\nabla_{\x} \dt \phi =0\\
&\ds\dt \intx\intrrr \left(\frac{\mid \vv\mid^2}{2}\f\right)+\frac12 \dt \intx \mid\nabla_{\x} \phi \mid^2 =0\\
\end{align*}
\end{proof}
...
\subsubsection{Test case 1: Laudau damping}
This is a classical test case for a non collision equation. The initial distribution function and electric field are given by 
$$
\left\{ \begin{array}{l}
f(t=0,x,v)=(1+\eps \cos(k x))\frac{1}{\sqrt{2\pi}}e^{-\frac{v^2}{2}}\\
E(t=0,x)=\frac{\eps}{k}\sin(k x)
\end{array}\right.
$$
with $L=\frac{2\pi}{k}$.
\subsubsection{Test case 2: two streaming instability}
This is a classical test case for a non collision equation. The initial distribution function is given by
$$
\left\{ \begin{array}{l}
f(t=0,x,v)=(1+\eps \cos(k x))\frac{1}{2\sqrt{2\pi}}\left(e^{-\frac{(v-v_0)^2}{2}}+e^{-\frac{(v+v_0)^2}{2}}\right)\\
\end{array}\right.
$$
with $v_0>0$.
This two test allows to validate the code for linear entropy (previous paper) and for the nonlinear entropy. 
\subsubsection{Test case 3: Riemann problem in the collision and non collision case}
This test case is explained in \cite{shinfil}. This is a classical Riemann problem which admet discontinuity solution in the collisional limit. The initial data is given by
$$
\left\{ \begin{array}{l}
(\rho_L, u_L, p_L)=(1, M\sqrt{2},1),\quad x\leq 0\\
(\rho_R, u_R, p_R)=(1, 0,1.05),\quad x > 0
\end{array}\right.
$$
with $M=2.5$.
\subsubsection{Test case 4: Hydrostatic steady state}
The hydrostatic steady state is a steady solution of the Euler equation with gravity potential (collisional limit of the Vlasov poisson equation). The Euler equation are given by
$$
\left\{\begin{array}{l}
\dt \rho + \dx(\rho u)=0\\
\dt (\rho u)+ \dx(\rho u^2)+ \dx p=\rho \dx \phi\\
\dt (\rho e )+ \dx(\rho u e)+\dx(pu) =\rho u \dx \phi
\end{array}\right.
$$
To obtain the steady equation we neglect the time derivative consequently we obtain a subset of solution given by $\dx(\rho u)=0$ and $\dx p=\rho\dx \phi$. an example of steady state is given by $u=0$, $\rho=1$ and $p=\phi+C$. Consequently the distribution (in the collisional limit) is given by
$$
f(t,x,v)=\frac{1}{2\pi (\phi +C)^{\frac12}}\exp^{\frac{-\mid v\mid^2}{2(\phi+c)}}
$$
It is not clear if this solution is a stationary solution of the Vlasov equation.
\subsubsection{Expansion plasma in a rarefied regime}
This test case is described in \cite{VPBgk_adap}. We consider a domain $[0\times 1]$, the left part in a that we can describe with a kinetic approximation (low collisional and lot a of basis function) and the write with hydrodynamic approximation (lot of collision and few basis function). The velocity domain is $[-8,8]$. The initial value is given by
 $$
\left\{ \begin{array}{l}
(\rho_L, u_L, T_L)=(1, 1, 1),\quad x\leq 0\\
(\rho_R, u_R, T_R)=(0.1, 0, 0.5),\quad x > 0
\end{array}\right.
$$
Consequently the left version this is the boundary condition. The poisson problem mi given by
 $$
\left\{ \begin{array}{l}
\Delta \phi =\rho -\rho_R,\mbox{ on } ]0,1[\\
\phi(0)=0.1,\mbox{ }\phi(1)=0
\end{array}\right.
$$
The collisional  coefficient is given by $\eps=\frac{0.001}{\rho}$. An idea is to compare a full hydrodynamique flow (use some number of basis functions ) and a kinetic-fluid flow (more basis functions in the kinetic part).
...
\bibitem{Quasineu} N. Crouseilles, A. Ratnani, E. Sonnendrücker, \emph{An Isogeometric Analysis approach for the study of the gyrokinetic quasi-neutrality equation} JCP 231 (2012) 373-393
...
\bibitem{harmonicsscheme}  D. Radice, E. Abdikamalov, L. Rezzolla, C. D. Ott \emph{A new spherical harmonics scheme for multi-dimensional radiation transport I: Static Matter configurations}, preprint
 ...
 \bibitem{reducedvlasov1} Helluy P., Pham N., Crestetto A., \emph{Space-only hyperbolic approximation of the Vlasov equation}, 2012  
  ...
  \bibitem{shinfil} F. Filbet, S. Jin, \emph{AN ASYMPTOTIC PRESERVING SCHEME FOR THE ES-BGK MODEL OF THE BOLTZMANN EQUATION}, ....
  
  \bibitem{VPBgk_adap} G. Dimarco, L. Mieussens, V. Rispoli \emph{An asymptotic preserving automatic domain decomposition method for the Vlasov?Poisson?BGK system with applications to plasmas}, JCP 274 (2014), 122-139
 
\bibitem{filtering}  A. J. Jerri \emph{The Gibbs Phenomena in Fourier Analysis, Splines and Wavelet approximations}, Mathematics and its applications
